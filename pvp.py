...
class PVP(ABC):
    ...
    def encode(self, example: InputExample, priming: bool = False, labeled: bool = False,
        ...
        if not tokens_b and self.model_name.startswith('bert'):
...
### VERBALIZER C NOT USABLE ###
    ...
    def verbalize(self, label) -> List[str]:
        ...
        if 'roberta' in self.model_name:
            XnliPVP.VERBALIZER_B[0] = ['right']
            XnliPVP.VERBALIZER_B[1] = ['maybe']
            XnliPVP.VERBALIZER_B[2] = ['wrong']
...
class PawsxPVP(PVP):
    ...
    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 0 or self.pattern_id == 1:
            if 'roberta' in self.model_name:
                PawsxPVP.VERBALIZER_A[0] = ['wrong']
                PawsxPVP.VERBALIZER_A[1] = ['right']
...
class AmazonPVP(PVP):
    ...
    def verbalize(self, label) -> List[str]:
        if 'roberta' in self.model_name:
            AmazonPVP.VERBALIZER[0] = ['ter']
        return AmazonPVP.VERBALIZER[label]


class AmazonPolarityPVP(PVP):
    VERBALIZER = {
        0: ["bad"],
        1: ["great"]
    }
    ...
    def verbalize(self, label) -> List[str]:
        return AmazonPolarityPVP.VERBALIZER[label]
...
class YahooPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        # pattern 2: [MASK] News: Germany won the 2014 Wolrd Cup.
            ...
            return [self.mask, 'Quetion', ':', text_a, text_b], []
        # pattern 3: Germany won the 2014 Wolrd Cup. Category: [MASK]
        elif self.pattern_id == 3:
            return [text_a, text_b, 'Category', ':', self.mask], []
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
    ...
    def verbalize(self, label) -> List[str]:
        if 'roberta' in self.model_name:
            YahooPVP.VERBALIZER[0] = ['Social']
            YahooPVP.VERBALIZER[7] = ['Fun']
...
class ColaPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return [text_a, '. It is linguistically', self.mask, '.'], []
        elif self.pattern_id == 1:
            return [text_a, '. It is grammatically', self.mask, '.'], []
        elif self.pattern_id == 2:
            return [text_a, '. The sentence is linguistically', self.mask, '.'], []
        elif self.pattern_id == 3:
            return [text_a, '. The sentence is grammatically', self.mask, '.'], []
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
...
class SST2PVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return ['It was', self.mask, '.', text_a], []
        elif self.pattern_id == 1:
            return [text_a, '. All in all, it was', self.mask, '.'], []       
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
...
class MrpcPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0 or self.pattern_id == 1:
            ...
            return [text_a, '.', text_b, '.'], ['Do both sentences have the same meaning ?', self.mask]
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
...
class QqpPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return ['Question 1:', text_a, self.mask, 'Question 2:', text_b], []
        elif self.pattern_id == 1:
            return ['Question 1:', text_a, 'Question 2:', text_b], ['Question 1 and Question 2 are', self.mask]
        elif self.pattern_id == 2 or self.pattern_id == 4:
            return [text_a, '?'], [self.mask, ',', text_b]
        elif self.pattern_id == 3 or self.pattern_id == 5:
            return ['Question 1:', text_a, '?'], [self.mask, ',', 'Question 2:', text_b]
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
    ...
    def verbalize(self, label) -> List[str]:
        ...
        return QqpPVP.VERBALIZER_C[label]
...
class RtePVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            ...
            return [text_a], ['question:', text_b, 'wrong or right?', self.mask]
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
...
class QnliPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0 or self.pattern_id == 1:
            return [text_a, '?', text_b, '.'], ['Does the question entail the answer?', self.mask]
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
...
class WnliPVP(PVP):
    ...
    def verbalize(self, label) -> List[str]:
        if self.pattern_id == 1:
            return WnliPVP.VERBALIZER_A[label]
        return WnliPVP.VERBALIZER_B[label]
        

PVPS = {
    'xnli': XnliPVP,
    'pawsx': PawsxPVP,
    'amazon_polarity': AmazonPolarityPVP,
    'amazon_star': AmazonPVP,
    "ag_news": AgNewsPVP,
    "yahoo": YahooPVP,
    'cola': ColaPVP,
    'sst2': SST2PVP,
    ...
    'qnli': QnliPVP,
    'wnli': WnliPVP,
    'ag_news_multi': AgNewsPVP,
    'amazon_reviews_multi': AmazonPVP,
    'xnli_multi': XnliPVP,
    'pawsx_multi': PawsxPVP
}