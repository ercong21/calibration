...
import json
import logging
from transformers import XLMTokenizer, PreTrainedTokenizer
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, language=None, meta: Optional[Dict]=None):
        self.guid = guid
        self.text_a = text_a
        self.text_b = text_b
        self.label = label
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if isinstance(tokenizer, XLMTokenizer):
            inputs = tokenizer.encode_plus(example.text_a, example.text_b, add_special_tokens=True,
                                           max_length=max_length, lang=example.language)
        else:
            inputs = tokenizer.encode_plus(example.text_a, example.text_b, add_special_tokens=True,
...
def normalization(emb: np.ndarray) -> np.ndarray:
    """
    :param emb: should be a 2d array
    """
    return emb / np.linalg.norm(emb, axis=1, keepdims=True)

def cosine_similarity(emb1: np.ndarray, emb2: np.ndarray) -> np.ndarray:
    """
    :param emb1: should be a 2d array (m*d)
    :param emb2: should be a 2d array (n*d)
    :return: a 2d array (m*n)
    """
    norm_emb1 = normalization(emb1)
    norm_emb2 = normalization(emb2)
